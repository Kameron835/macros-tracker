...
function scaleFoodNutrition(food: BarcodeFood, grams: number) {
  const ratio = grams / Number(food.serving_size_grams || 100)
...
export async function importFoodByBarcode(input: ImportFoodByBarcodeInput) {
  ...
  const { data: existingFood, error: existingFoodError } = await supabase
    .from('foods')
    .select(foodSelect)
    .eq('source', 'Open Food Facts')
    .eq('source_id', cleanedBarcode)
    .maybeSingle()
  ...
  let food = existingFood as BarcodeFood | null
  ...
  if (!food) {
    ...
    const name =
      product.product_name?.trim() || `Barcode food ${cleanedBarcode}`
    ...
    if (insertError || !insertedFood) {
      throw new Error(insertError?.message || 'Could not import food.')
    }

    food = insertedFood as BarcodeFood
  }
  ...
  if (input.addToMeal) {
    ...
    const scaledNutrition = scaleFoodNutrition(food, grams)
